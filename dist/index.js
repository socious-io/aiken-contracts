...
import {
  ...
  MeshTxBuilder as MeshTxBuilder2
} from "@meshsdk/core";
...
var MeshEscrowContract = class extends MeshTxInitiator {
  ...
  getScriptCbor = () => {
    return applyParamsToScript(plutus_default.validators[0].compiledCode, []);
  };
  ...
  recipientDeposit = async (escrowUtxo, depositAmount) => {
    ...
    await this.mesh.spendingPlutusScript(this.languageVersion).txIn(escrowUtxo.input.txHash, escrowUtxo.input.outputIndex, escrowUtxo.output.amount, this.scriptAddress).spendingReferenceTxInInlineDatumPresent().txInRedeemerValue(recipientDepositRedeemer(walletAddress, depositAmount), "JSON", DEFAULT_REDEEMER_BUDGET).txInScript(this.scriptCbor).txOut(this.scriptAddress, escrowAmount).txOutInlineDatumValue(outputDatum, "JSON").changeAddress(walletAddress).txInCollateral(
      collateral.input.txHash,
      collateral.input.outputIndex,
      collateral.output.amount,
      collateral.output.address
  ...
  completeEscrow = async (escrowUtxo) => {
    ...
    await this.mesh.spendingPlutusScript(this.languageVersion).txIn(escrowUtxo.input.txHash, escrowUtxo.input.outputIndex, escrowUtxo.output.amount, this.scriptAddress).spendingReferenceTxInInlineDatumPresent().spendingReferenceTxInRedeemerValue(mConStr2([])).txInScript(this.scriptCbor).txOut(initiatorAddress, initiatorToReceive).txOut(recipientAddress, recipientToReceive).requiredSignerHash(deserializeAddress(recipientAddress).pubKeyHash).requiredSignerHash(deserializeAddress(initiatorAddress).pubKeyHash).changeAddress(walletAddress).txInCollateral(
      collateral.input.txHash,
      collateral.input.outputIndex,
      collateral.output.amount,
      collateral.output.address
...
var demo = async () => {
  ...
  const initiatorWallet = new MeshWallet2({
    networkId: 0,
  ...
  const meshTxBuilder = new MeshTxBuilder2({
    fetcher: blockchainProvider,
    submitter: blockchainProvider,
    verbose: true
  });
